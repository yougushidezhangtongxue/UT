...
function sys_ShowListMorePage($num,$page,$dolink,$type,$totalpage,$line,$ok,$search="",$add){
	...
	if($page<>1)
	{
		$toppage='<li class="prev-page"><a href="'.$dolink.$add['dofile'].$type.'">'.$fun_r['startpage'].'</a></li>';
		$pagepr=$page-1;
		if($pagepr==1)
		{
		...
		$prepage='<li class="prev-page"><a href="'.$dolink.$prido.'">'.$fun_r['pripage'].'</a></li>';
	}
	//下一页
	if($page!=$totalpage)
	{
		...
		$nextpage='<li class="next-page"><a href="'.$nextpagelink.'">'.$fun_r['nextpage'].'</a></li>';
		$lastpage='<li class="next-page"><a href="'.$lastpagelink.'">'.$fun_r['lastpage'].'</a></li>';
	}
...
function sys_ShowTextPage($totalpage,$page,$dolink,$add,$type,$search=""){
	...
	if($page<>1)
	{
		$toppage='<li class="prev-page"><a href="'.$dolink.$add[filename].$type.'">'.$fun_r['startpage'].'</a></li>';
		$pagepr=$page-1;
		if($pagepr==1)
		{
	...
	{
		$pagenex=$page+1;
		$nextpage='<li class="next-page"><a href="'.$dolink.$add[filename].'_'.$pagenex.$type.'">'.$fun_r['nextpage'].'</a></li>';
		$lastpage='<li class="next-page"><a href="'.$dolink.$add[filename].'_'.$totalpage.$type.'">'.$fun_r['lastpage'].'</a></li>';
	}
	$starti=$page-$snum<1?1:$page-$snum;
	$no=0;
	for($i=$starti;$i<=$totalpage&&$no<$page_line;$i++)
	{
		...
		elseif($i==1)
		{
			$is_1='<li><a href="'.$dolink.$add[filename].$type.'">';
			$is_2="</a></li>";
		}
		else
		{
			$is_1='<li><a href="'.$dolink.$add[filename].'_'.$i.$type.'">';
			$is_2="</a></li>";
		}
		$returnstr.='&nbsp;'.$is_1.$i.$is_2;
	}
