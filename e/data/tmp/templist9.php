    ...
    <div class="content">
      <div id="wowslider-container1">
        <div class="ws_images">
          ...
          <?php
$bqno=0;
$ecms_bq_sql=sys_ReturnEcmsLoopBq('news',10,20,1,'isgood=1','newstime DESC');
if($ecms_bq_sql){
while($bqr=$empire->fetch($ecms_bq_sql)){
$bqsr=sys_ReturnEcmsLoopStext($bqr);
$bqno++;
?>
            <li><a target="_blank" href="<?=$bqsr['titleurl']?>" title="<?=$bqr['title']?>"><img src="<?=sys_ResizeImg($bqr[titlepic],716,296,1,'')?>" title="<?=$bqr['title']?>" alt="<?=$bqr['title']?>" /></a></li>
          <?php
}
}
?>
...
$bqno++;
?>
          <a target="_blank" href="#" title="<?=$bqr['title']?>"><img src="<?=sys_ResizeImg($bqr[titlepic],120,62,1,'')?>" /></a>
          <?php
}
}
?>
      ...
      <script type="text/javascript" src="/skin/ecms008/js/slider.js"></script>
      <div class="banner banner-sticky"><script type="text/javascript" src="/d/js/acmsd/thea7.js"></script></div>
      <div class="hot-posts">
        <h2 class="title">热门排行</h2>
        <ul>
        <?php
$bqno=0;
$ecms_bq_sql=sys_ReturnEcmsLoopBq('news',5,19,0,'','newstime DESC');
if($ecms_bq_sql){
while($bqr=$empire->fetch($ecms_bq_sql)){
$bqsr=sys_ReturnEcmsLoopStext($bqr);
$bqno++;
?>
          <li>
            <p><span class="ds-thread-count post-comments" data-thread-key="<?=$bqr['id']?>">暂无评论</span><span class="muted"><a href="JavaScript:makeRequest('[!--news.url--]e/public/digg?classid=<?=$bqr['classid']?>&id=<?=$bqr['id']?>&dotop=1&doajax=1&ajaxarea=diggnum<?=$bqr['id']?>','EchoReturnedText','GET','');"  class="action"><i class="fa fa-heart-o"></i><span class="count" id="diggnum<?=$bqr['id']?>"><script src="[!--news.url--]e/public/ViewClick?classid=<?=$bqr['classid']?>&id=<?=$bqr['id']?>&down=5"></script></span>喜欢</a></span></p>
            <span class="label label-1"><?=$bqno?></span><a target="_blank" href="<?=$bqsr['titleurl']?>" title=""><?=$bqr['title']?></a></li>
            <?php
}
}
?>
      ...
      <div class="pagination">
        <ul>
        [!--show.listpage--]
<li class="next-page"><a href="/index_2.html">下一页</a></li></li>
        </ul>
      </div>
    </div>
  </div>
